...
if st.session_state['login_status']:
    ...
    with tab1:
        with st.form(key='rating_form'):
            ...
            st.text('L\'attrazione fisica ed estetica')
            rating_i = st_star_rating("Interazione", maxValue=5, defaultValue=3, key="rating_i")
            st.text('La qualità dell\'interazione nel gioco di conoscenza')
            rating_v = st_star_rating("Interessi comuni", maxValue=5, defaultValue=3, key="rating_v")
            st.text('Il potenziale di proseguire con il reciproco interesse')
